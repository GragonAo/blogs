...
class Http {
  public static baseURL = 'http://127.0.0.1:8000/'; // 静态基础 URL，可以按需修改  
  ...
  //   // console.log(useUserStore().userLoginInfo)
  //   // if (useUserStore().userLoginInfo?.token != undefined) {
  //   //   return useUserStore().userLoginInfo!.token;
  //   // }
  //   return "";
  // }
  private httpRequestInterceptor(config: AxiosRequestConfig<any>): InternalAxiosRequestConfig<any> {
    ...
    const token = useUserStore().userLoginInfo?.token; // 尝试获取 token  
    // 2. 请求超时时间，默认20s  
